...
minimax(E,Prof,Is_Maximizing,BestMov,Value) :-
    minimax(E,Prof,Is_Maximizing,BestMov,Value,2,10).

minimax(E,Prof,Is_Maximizing,BestMov,Value,Jugador,Alfa) :-
    Prof > 0,
    NewProf is Prof-1,
    findall(mv(Mov,0),mover(E,Mov,_),MovsPosibles),
    length(MovsPosibles,Len),
    MovsFunc =.. [movs|MovsPosibles],
    recursive(E,MovsFunc,Len,NewProf,Is_Maximizing,Jugador,Alfa),
    MovsFunc =.. [_|Movs],
...
minimax(E,0,_Is_Maximizing,_,Value,Jugador,_Alfa) :-
...
recursive(E,MovsF,Len,Prof,Is_Maximizing,Jugador,Alfa) :-
    (Is_Maximizing = true -> 
        between(1,Len,I),
        arg(I,MovsF,mv(Mov,_)),
        mover(E,Mov,E2),
        minimax(E2,Prof,false,_,Value,Jugador,Alfa),
        Alfa =< Value,!
        ;between(1,Len,I),
        arg(I,MovsF,mv(Mov,_)),
        mover(E,Mov,E2),
        minimax(E2,Prof,true,_,Value,Jugador,Alfa),
        Alfa >= Value,!),
    nb_setarg(I,MovsF,mv(Mov,Value)),
    fail.

recursive(_,_,_,_,_,_,_).

distancia_al_arco(E,N) :-
    posicion_pelota(E,p(_X,Y)),
    arg(3,E,Turno),
    arg(1,Turno,Jugador),
    ((Jugador > 1) = true -> N is 5-Y;
        N is 5+Y).
